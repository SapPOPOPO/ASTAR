...
class Random(object):
    ...
    def __init__(self, tao=0.2, gamma=0.7, beta=0.2, \
        ...
        if self.augment_threshold == -1:
            self.data_augmentation_methods = [Crop(tao=tao), Mask(gamma=gamma, mask_id=mask_id), Reorder(beta=beta), 
                                Insert(item_similarity_model, insert_rate=insert_rate, 
                                    max_insert_num_per_pos=max_insert_num_per_pos),
                                Substitute(item_similarity_model, substitute_rate=substitute_rate)]
...
class RRandom(object):
    ...
    def __init__(self, tao=0.2, gamma=0.7, beta=0.2, \
        ...
        self.data_augmentation_methods = [Mask(gamma=gamma, mask_id=mask_id), Reorder(beta=beta)]
        print("Total augmentation numbers: ", len(self.data_augmentation_methods))
...
import torch
class Reeorder(object):
    """Randomly shuffle a continuous sub-sequence — works on torch tensors"""
    def __init__(self, beta=0.2):
        self.beta = beta

    def __call__(self, sequence):
        # sequence is expected to be 1D torch tensor
        ...
        start_index = random.randint(0, seq_len - sub_seq_length)
        end_index = start_index + sub_seq_length
        
        # extract sub-sequence
        sub_seq = sequence[start_index:end_index].clone()
        
        # shuffle it (tensor version)
        perm = torch.randperm(sub_seq_length)
        shuffled_sub = sub_seq[perm]
        ...
        assert reordered.shape[0] == seq_len, "Length changed!"
        return reordered